...
export const useVehicle = () => {
  ...
  const createVehicle = useCallback(async (request: CreateVehicleRequest) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Không thể tạo phương tiện';
  ...
  const updateVehicle = useCallback(async (vehicleId: string, request: UpdateVehicleRequest) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Không thể cập nhật phương tiện';
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
      setLoading(false);
  ...
  const getVehicleById = useCallback(async (vehicleId: string) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Không thể tải thông tin phương tiện';
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
      setLoading(false);
  ...
  const getAllVehicles = useCallback(async (filters?: VehicleFilters) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Không thể tải danh sách phương tiện';
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
      setLoading(false);
  ...
  const getAvailableVehicles = useCallback(async (filters: AvailableVehicleFilters) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const getAvailableForBooking = useCallback(async (filters: AvailableVehicleFilters) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const deleteVehicle = useCallback(async (vehicleId: string) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Không thể xóa phương tiện';
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
      setLoading(false);
  ...
  const changeVehicleStatus = useCallback(async (vehicleId: string, status: VehicleStatus) => {
    setLoading(true);
    setError(null);
    try {
      const data = await vehicleService.changeVehicleStatus(vehicleId, status);
      return { success: true, data };
    } catch (err: any) {
  ...
  const uploadVehiclePhotos = useCallback(async (vehicleId: string, files: File[]) => {
    setLoading(true);
    setError(null);
    try {
